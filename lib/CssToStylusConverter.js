...
class CssToStylusConverter {
	...
	concat (node, level) {
		...
		if (children.size) {
			...
			children.forEach((node, key) => {
				if (this.separateRules && i === 1 && rules.size)
					source.push('');

				this.concat(node, level + 1);

				if (this.separateRules && i < children.size)
					source.push('');
		...
		if (this.separateRules && level === 1)
			source.push('');
	}

	_getUniqNodeRules(node) {
		return [...node.get('rules')].reduce((newSet, value) => {
			if (!this._hasSameDeclaration(newSet, value))
				newSet.add(value);
	...
	_hasSameDeclaration(checkedSet, decl) {
		return [...checkedSet].some((value) => {
			return value.property === decl.property &&
	...
	walkRules (rule, root) {
		switch (rule.type) {
			...
			break;
			case 'keyframes':
				if (this.unprefix && !_.isUndefined(rule.vendor))
					return false;
		...
		parts.reduce((children, selector, idx, arr) => {
			...
			if (idx === arr.length - 1 && rule.declarations) {
				rule.declarations.forEach(decl => {
					if (this.removeComments && decl.type === 'comment')
						return false;
					...
					if (decl.type === 'declaration') {
						...
						if (decl.property === 'filter' && decl.value.match(/alpha|microsoft/i))
							return false;

						if (this.unprefix) {
							const vendorPrefixR = /^-\w+-(.+)$/;

							if (vendorPrefixR.test(decl.property))
								decl.property = decl.property.replace(vendorPrefixR, '$1');

							if (vendorPrefixR.test(decl.value))
								return false;
						}

						if (this.createColorVariables) {
							const colorRaised = this.colorRaiser.raise(decl.property, decl.value);

							if (colorRaised !== false)
								decl.value = colorRaised;
						}
					}
	...
	processDeclaration (rule, root) {
		rule.selectors.some((path, idx, arr) => {
			...
			if (buff.length)
				path = `${_.trimEnd(path)}${buff.reverse().join('')}`;
