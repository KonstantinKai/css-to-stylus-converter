...
module.exports = function convert (cssSource, options = {}) {
	...
	function concat (node, level = 1) {
		source.push(`${_.repeat(indent, level - 1)}${node.get('declaration')}`);
		...
		if (rules.size) {
			if (cssSyntax) source[source.length - 1] += ` ${openBracket}`;
			...
			rules.forEach(rule => {
				switch (rule.type) {
					case 'declaration':
						...
						source.push(`${_.repeat(indent, level)}${rule.property}${colon} ${ruleValue}`);
					break;
					case 'comment':
						if (removeComments) return false;
